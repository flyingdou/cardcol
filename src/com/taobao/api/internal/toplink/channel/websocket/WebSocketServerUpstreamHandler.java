...
public class WebSocketServerUpstreamHandler extends NettyServerUpstreamHandler {
	...
	private void handleHttpRequest(ChannelHandlerContext ctx, HttpRequest req) {
		...
		if (this.channelHandler != null) {
			try {
				...
				// this.channelHandler.onConnect(this.createContext(ss)); :
				// 通过编译, bug未解决
				...
				this.logger.error(e);
				this.sendHttpResponse(ctx, req,
						new DefaultHttpResponse(HttpVersion.HTTP_1_1, new HttpResponseStatus(401, e.getMessage())));
				return;
			}
		}
	...
	private void dump(HttpRequest request) {
		...
		// Entry<String, String> h : request.getHeaders() : 原错误代码,
		// List<Entry<String, String>> ss = null; 和
		// Entry<String, String> h : ss : 通过编译, bug未解决
		List<Entry<String, String>> ss = null;
	...
	private void renderServerChannelContext(HttpRequest request) {
		...
		// Entry<String, String> h : request.getHeaders() : 原错误代码,
		// List<Entry<String, String>> ss = new ArrayList<Entry<String, String>>(); ss = null; 和
		// Entry<String, String> h : ss : 通过编译, bug未解决
		List<Entry<String, String>> ss = new ArrayList<Entry<String, String>>();
		for (Entry<String, String> h : ss)
