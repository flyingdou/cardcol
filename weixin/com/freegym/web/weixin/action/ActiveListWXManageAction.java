...
@Results({ @Result(name = "success", location = "/WX/login.jsp"),
		...
		@Result(name = "activeall", location = "/WX/join_challenge.jsp"),// 挑战列表页
		@Result(name = "execut", location = "/WX/challenge_detail.jsp"),// challengeXq.jsp 原来的页面
		...
		@Result(name = "joinchallenge", location = "/WX/joinChallenge2.jsp") })  // 参加挑战页面
public class ActiveListWXManageAction extends BaseBasicAction {
			...
			if (pageInfo.getItems() != null && pageInfo.getItems().size() > 0) {
				for (final Iterator<?> it = pageInfo.getItems().iterator(); it.hasNext();) {
					...
					obj.accumulate("id", a.getId()).accumulate("name", a.getName())
							...
							.accumulate("category", a.getCategory()).accumulate("value",a.getValue());
					jarr.add(obj);
				}
			}
