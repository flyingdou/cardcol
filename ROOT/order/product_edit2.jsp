...
$(document).ready(function(){
	...
	$("input[name='product.num']").formValidator().inputValidator({min:1,max:20,onError:"健身次数不能为空,请确认"}).regexValidator({regExp:"^[1-9][0-9]*$",onError:"健身次数必须为正整数,请确认"});
	...
	$("input[name='product.delayDay']").formValidator().inputValidator({min:1,max:20,onError:"3-5天的合理健身频率不能为空,请确认"}).regexValidator({regExp:"^[1-9][0-9]*$",onError:"3-5天的合理健身频率数必须为正整数,请确认"});
...
<s:hidden name="product.image3"/>
   ...
   <div class="proedit">
	...
	<div class="proedifour">
		...
		<p>
			1.甲方承诺从<input type="text" name="product.startTime" id="startTime" class="text1" value="由会员填写" readonly="true" style="background: #f5f5f5;"/>起购买乙方的  <s:textfield name="product.num" cssClass="text2"/> 次  计次卡，成为乙方会员。
		</p>
		<p>
		...
		<p>
			3.该会员资格的有效期限为 <s:textfield name="product.wellNum" cssClass="text2"/> 月；有效期限届满后，卡内仍有剩余次数的，乙方应当提供一次展期，展期时间计算方式：剩余次数乘以 <s:textfield name="product.delayDay" cssClass="text2"/> 天（3-5天的合理健身频率）=展期天数。
		</p>
		<p>
			4.甲方应向乙方支付合同款共计<s:textfield name="product.cost" cssClass="text2"/> 元。双方同意将该合同款交付给丙方托管，并由丙方每月<input type="text" name="productReportDay" class="text1" value="系统自动计算" readonly="true" style="background: #f5f5f5;"/>号进行上一个月甲乙双方应付费用的结算。
		</p>
		<p>
