...
<s:hidden name="product.id" id="productId"/>
    ...
    <div>
      <div class="div1"> 交易起止日期
        ...
        <s:select name="query.status" list="#{'0':'未确认','1':'履约中','2':'已完成','3':'已结算'}" listKey="key" listValue="value" headerKey="" headerValue="请选择" cssClass="date-2"/>
      ...
      <div class="div2">
        <table width="100%" border="0" cellpadding="0" cellspacing="0"  style="white-space:normal">
          <tr class="end">
            ...
            <th id="tdtop" style="width:120px;">商品名称</th>
			<th id="tdtop" style="width: 70px;text-align:center;">
				<s:if test="queryType==\"1\"">买方</s:if>
				<s:else>卖方</s:else></th>
			<th id="tdtop" style="width:50px;">金额</th>
            <th id="tdtop" style="width:144px;">下单时间</th>
            <th id="tdtop" style="width:58px;">订单状态</th>
            <s:if test="type == 1 && queryType == 2">
          ...
          <tr>
            ...
            <td class="left" style="width:100px;">
            	<div style="width:100px;word-break:break-all;padding:0 10px;text-align: center;">
           	 		<s:property value="no"/>
            	</div>
            </td>
            <td style="width:80px;white-space:normal;word-wrap：break-word;"><div style="width:80px;word-break:break-all;text-align: center;"><s:property value="orderType"/></div></td>
            <td style="width:120px;white-space:normal;word-wrap：break-word;"><div style="word-break:break-all;overflow:hidden;text-align: center;">
            <s:if test="type==\"1\" || type==\"2\"|| type==\"3\" || type==\"6\""><a href="javascript:productDetail(<s:property value="productId"/>,<s:property value="type"/>);"><s:property value="name"/></a></s:if><s:else><s:property value="name"/></s:else>
            </div></td>
            <td style="width:70px;"><div style="word-break:break-all;text-align: center;"><s:if test="queryType == \"1\""><s:property value="fromName"/></s:if><s:else><s:property value="toName"/></s:else><div></td>
            <td style="width:50px;white-space:normal;word-wrap：break-word "><div style="width:50px;word-break:break-all;text-align: center;"><s:property value="orderMoney"/></div></td>
            <td style="width:58px;white-space:normal;word-wrap：break-word "><div style="word-break:break-all;text-align: center;"><s:date name="orderDate" format="yyyy-MM-dd"/></div></td>
            <td style="width:58px;">
	            <div style="word-break:break-all;text-align: center;">
		            <s:if test="status==\"0\"">未付款</s:if>
		          	<s:if test="status==\"1\"">已付款</s:if>
		          	<s:if test="status==\"2\"">已结束</s:if>
		          	<s:if test="status==\"3\"">已结算</s:if>
	          	</div>
            </td>
            <s:if test="type2 == 1 && queryType == 2">
            	<td class="right"><div style="word-break:break-all;text-align: center;">
	            	<s:if test="status==\"0\"">
