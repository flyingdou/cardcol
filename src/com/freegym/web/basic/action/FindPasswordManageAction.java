...
public class FindPasswordManageAction extends BaseBasicAction {
	...
	public void updatepassword(){
		...
		if (isRightful(mobile, code)) {
			if (member.getId()!= null) {
				...
				try {
					response.sendRedirect("login.asp?updateStatus=1");
				} catch (IOException e) {
					response("ERROR"); 
				};
			} else {
			...
			try {
				response.sendRedirect("login.asp?updateStatus=0");
			} catch (IOException e) {
				response("ERROR"); 
			}
		}
