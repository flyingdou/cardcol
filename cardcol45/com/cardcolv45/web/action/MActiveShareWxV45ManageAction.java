...
@Results({
	@Result(name="ordercoach",location="/coachorder.jsp"),
...
public class MActiveShareWxV45ManageAction extends BasicJsonAction {
	...
	public void list() {
		...
		request1.put("addornot", a.getCategory().equals('A') ? "体重增加" :a.getCategory().equals('B')?"体重减少":a.getCategory().equals('E')?"运动": a.getCategory().equals('D')?"运动":"");
	...
	public String trainrecord() {
		...
		List<Body> b=(List<Body>) service.findObjectBySql("select conclusion from Body as b where b.member = (select partake from TrainRecord as tr where tr.id=?)",id);
