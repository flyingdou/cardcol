...
@Results({ @Result(name = "listPlans", location = "/wxv45/list_plans.jsp"),
		@Result(name = "planDetail", location = "/wxv45/plan_detail.jsp"),
		@Result(name = "orderinfo", location = "/wxv45/orderinfo1.jsp"),
		@Result(name = "myPlans", location = "/wxv45/my_plan.jsp"), // 我的计划
		@Result(name = "myPlanDetail", location = "/wxv45/my_plan_detail.jsp"), // 我的计划
		@Result(name = "listCourse", location = "/wxv45/course_list.jsp"), // 课程列表
		@Result(name = "courseDetail", location = "/wxv45/course_detail.jsp"), // 课程详情
		@Result(name = "courseAppointment", location = "/wxv45/course_appointment.jsp") // 课程详情
})
public class MCourseWxV45ManageAction extends CourseJsonAction {

	private static final long serialVersionUID = -4724815615241826556L;
	...
	public String courses() {
		...
		memberTest.setId(Long.parseLong("4781"));
		memberTest.setRole("M");
		request.getSession().setAttribute("member", memberTest);
		id = Long.parseLong("334");
		role = "S";
		String time = "2022-10-22";

		SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd");
		try {
			planDate = formatter.parse(time);
		} catch (Exception e) {
			log.error("error", e);
		}
		...
		final String sDate = formatter.format(ds[0]);
		final String eDate = formatter.format(ds[1]);
		List<?> list = null;
		boolean hasRel = true;
		final Member mu = (Member) request.getSession().getAttribute("member");
		if (id == null) { // 这里是自身访问，取得登录用户的课表
			...
			// 判断有无关系
			hasRel = service.hasRelation(mu.getId(), id);
		...
		for (final Iterator<?> it = list.iterator(); it.hasNext();) {
			...
			obj.accumulate("id", c.getId()).accumulate("image", c.getCourseInfo().getImage())
					...
					.accumulate("member", getMemberJson(c.getMember())).accumulate("coach", getMemberJson(c.getCoach()))
					.accumulate("planDate", c.getPlanDate()).accumulate("startTime", c.getStartTime())
					.accumulate("endTime", c.getEndTime()).accumulate("courseId", c.getCourseInfo().getId())
					...
					.accumulate("hasPartake", hasPartake).accumulate("memberPrice", getDouble(c.getMemberPrice()))
	...
	public String courseDetail() {
		// TODO Test Data
		if (id == null) {
			id = Long.parseLong("117411");
		}

		List<?> list = service.findObjectBySql("from Course cs where cs.id = ?", new Object[] { id });
	...
	public String myPlans() {
		// TODO
		Member memberTest = new Member();
		memberTest.setId(Long.parseLong("978"));
		memberTest.setRole("M");
		request.getSession().setAttribute("member", memberTest);

		String time = "2015-05-17";
		...
		request.setAttribute("pageInfo", jarr);
		...
		try {
			...
			if (list.size() > 0) {
				...
				for (final Iterator<?> it = list.iterator(); it.hasNext();) {
					...
					int j = (int) d;
					obj.accumulate("id", wd.getId()).accumulate("planWeight", getString(wd.getPlanWeight()))
							.accumulate("actualWeight", getString(wd.getActualWeight()))
							.accumulate("planIntervalSeconds", getString(wd.getPlanIntervalSeconds()))
							.accumulate("actualIntervalSeconds", getString(wd.getActualIntervalSeconds()))
							.accumulate("planTimes", getString(wd.getPlanTimes()))
							.accumulate("actualTimes", getString(wd.getActualTimes()))
	 ...
	 *            image3: 计划图片三
	 * @param JSON数组
	 *            jsons:[{planName: 计划名称， planType： 计划类型 A.瘦身减重 B.健美增肌 C.运动康复
	 *            D.提高运动表现， scene： 适用场景 A.健身房 B.办公室 C.家庭 D.户外， applyObject： 适用对象
	 *            A.初级 B.中级 C.高级， apparatuses： 所需器材， startDate： 计划开始时间， endDate：
	 *            计划结束时间， unitPrice： 销售价格， briefing： 计划简介， details： 计划详情，
	 *            image1FileName： 计划图片一名称， image2FileName： 计划图片二名称，
	 *            image3FileName： 计划图片三名称， member：保存用户id}];"
	 * 
	...
	private String getActionJson(List<Action> acts) {
		...
		for (final Action a : acts) {
			final JSONObject obj = new JSONObject();
			obj.accumulate("id", a.getId()).accumulate("name", a.getName()).accumulate("image", getString(a.getImage()))
		...
		try {
			...
			if (3 == type) {
				...
				obj.accumulate("success", true).accumulate("item",
						plan.toJson().accumulate("memberName", member.getName()).accumulate("appraise",
								getGradeJson(plan.getId(), type + "")));
			} else if (6 == type) {
				final Goods good = (Goods) service.load(Goods.class, id);
				final Member member = (Member) service.load(Member.class, good.getMember());
				obj.accumulate("success", true).accumulate("item",
						good.toJson().accumulate("memberName", member.getName()).accumulate("appraise",
								getGradeJson(good.getId(), type + "")));
			}
	...
	public String listPlans() {
		try {
			// TODO Test Data
			Member memberTest = new Member();
			memberTest.setId(Long.parseLong("352"));
			memberTest.setRole("M");
			request.getSession().setAttribute("member", memberTest);
			userId = Long.parseLong("479");// 教练
			currentPage = 0;
			...
			if (!"S".equals(member.getRole())) {
				...
				if (null != userId) {
					...
					dc.add(Restrictions.or(
							Restrictions.and(Restrictions.ne("member.id", member.getId()),
									Restrictions.eq("isClose", "2"), Restrictions.eq("audit", "1")),
	...
	public String orderInfo() {
		...
		if (member == null) {
			...
			String sql = "SELECT plan_name orderName,unit_price orderCost FROM tb_plan_release WHERE ID = ?";
			pageInfo = service.findPageBySql(sql, pageInfo, new Object[] { 190 });
	...
	/**
	 * 关闭/开启健身计划，将id对应的健身计划，如果是1改为2，是2改为1
	 * 
	 * @param String
	 *            uuid: 用户唯一标识
	 * @param Long
	...
	/**
	 * 改变私教状态, 将id对应的私教课程，如果是1改为2，是2改为1
	 * 
	 * @param String
	 *            uuid: 用户唯一标识
	 * @param Long
	...
	public void showCourse() {
		...
		try {
			...
			if (keyword != null && !"".equals(keyword)) {
				dc.add(this.or(Restrictions.like("c.name", "%" + keyword + "%"),
						Restrictions.like("m.clubname", "%" + keyword)));
			}
	...
	public void showCourseDetail() {
		try {
			final Course c = (Course) service.load(Course.class, courseId);
			final JSONObject obj = new JSONObject();
			obj.accumulate("id", c.getId()).accumulate("courseName", c.getCourseInfo().getName())
					.accumulate("coach", c.getCoach().getName()).accumulate("planDate", c.getPlanDate())
					.accumulate("startTime", c.getStartTime()).accumulate("endTime", c.getEndTime())
					.accumulate("count", c.getCount()).accumulate("joinNum", c.getJoinNum())
					.accumulate("memberPrice", c.getMemberPrice()).accumulate("hourPrice", c.getHourPrice())
					.accumulate("courseMember", c.getMember().getId())
					.accumulate("address", getString(c.getMember().getAddress()))
					.accumulate("longitude", getDouble(c.getMember().getLongitude()))
					.accumulate("latitude", getDouble(c.getMember().getLatitude()))
