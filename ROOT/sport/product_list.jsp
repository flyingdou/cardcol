...
<body ontouchstart>
	...
	<div class="weui-cells" style="margin-bottom: 65px">
		<ul class="mui-table-view ul2" style="margin: 2px;" id="productList">
			<!-- 课程列表开始 -->
			<li class="mui-table-view-cell mui-media" v-for="(product,index) in products">
				<a :href="'scoursewx!productDetail.asp?id='+proudct.id">
					<img class="mui-media-object mui-pull-left user_img" :src="'picture/'+product.image" />
					<div class="mui-media-body">
						<div class="bt">{{product.name}}</div>
						<div class="bt">{{product.member.name}}</div>
						<p class="time">{{product.startTime}}-{{product.endTime}}</p>
					</div>
				</a>
			</li>
			<!-- 课程列表结束 -->
	...
	<div class="footer" id="footer">
		<a id="make" href="mcoursewxv45!goMake.asp">预约课程</a>
	</div>
	...
	<script type="text/javascript">
		$(function() {
			...
			$(".picker-calendar-day").click(function(){
				$.ajax({
					url:"scoursewx!products.asp",
					type:"post",
					data:{"planDate":$(this).attr("data-date")},
					dataType:"json",
					sync:true,
					success:function(res){
						productList.products = res.items;
					}
				});
			});
			if(data.isHave == false){
				$("#make").attr("href","javascript:void(0)");
			}
		});
	</script>
</body>
