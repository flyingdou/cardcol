...
public class MMessageManageAction extends BasicJsonAction {
	...
	protected void list() {
		...
		for (final Iterator<?> it = pageInfo.getItems().iterator(); it.hasNext();) {
			...
			obj.accumulate("id", msg.getId()).accumulate("parent", getLong(msg.getParent())).accumulate("type", getString(msg.getType()))
					.accumulate("isRead", getString(msg.getIsRead())).accumulate("memberFrom", getMemberJson(msg.getMemberFrom()))
					.accumulate("memberTo", getMemberJson(msg.getMemberTo())).accumulate("content", sdf.format(msg.getSendTime()) + " " +getString(msg.getContent())).accumulate("status", getString(msg.getStatus()));
			jarr.add(obj);
		}
