...
$(document).ready(function(e) {
    ...
    /*我的计划里日期选择效果*/
    $("#centerMain").on("click",".item", function (e) {
        if($(this).hasClass("lastItem")){
            e.stopPropagation()
        }else {
            $("#centerMain").find(".item").removeClass("currentItem");
            $(this).addClass("currentItem")
        }
    });

    var main=$("#centerMain").find(".item");
    if(main!=""){
        $("#centerMain").find(".item").addClass("main");
    }
});
...
var CalendarHandler = {
    ...
    CreateCalendar: function(y, m, d) {
        ...
        for (var i = 0; i < nowDaysNub; i++) {
            if (i == (nowDay - 1)) $dayItem.append("<div class=\"item currentItem thisDay \"><a>" + (i + 1) + "</a></div>");
