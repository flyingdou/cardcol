...
@Results({ @Result(name = "success", location = "/WX/login.jsp"), @Result(name = "bodys", location = "/wxv45/body.jsp") })
public class BodyDataWxV45ManageAction extends BaseBasicAction {
	/**
	 * 会员身体数据
	 */
	...
	public String bodys() {
		...
		if (member != null) {
			...
			if (trainRecord.getId() == null) {
				...
				// 如果今天已经保存过数据了
				String sql = "SELECT * from tb_plan_record cr where id = ?";
				pageInfo = service.findPageBySql(sql, pageInfo, trainRecord.getId());
			}
			
			// 获取会员的身高 
			final Setting setting = service.loadSetting(member.getId());
			LinkedCaseInsensitiveMap map =(LinkedCaseInsensitiveMap) pageInfo.getItems().get(0);
			map.put("height", setting.getHeight());
