...
public class WebSocketClientUpstreamHandler extends NettyClientUpstreamHandler {
	...
	private void handleHandshake(ChannelHandlerContext ctx, HttpResponse response)
		...
		// response.getHeader() : 原错误代码, HttpServletResponse response2 = null 和
		// response2.getHeader() : 通过编译, bug未解决
		HttpServletResponse response2 = null;
		boolean validUpgrade = response2.getHeader(Names.UPGRADE) != null &&
				response2.getHeader(Names.UPGRADE).equalsIgnoreCase(Values.WEBSOCKET);
		boolean validConnection = response2.getHeader(Names.CONNECTION) != null &&
				response2.getHeader(Names.CONNECTION).equalsIgnoreCase(Values.UPGRADE);
	...
	private void dump(HttpResponse response) {
		...
		// Entry<String, String> h : response.getHeaders() : 原错误代码, 
		// List<Entry<String, String>> ss = new ArrayList<Entry<String, String>>(); 和
		// Entry<String, String> h : ss : 通过编译, bug未解决
		List<Entry<String, String>> ss = new ArrayList<Entry<String, String>>();
		for (Entry<String, String> h : ss) {
