...
public class MMemberManageAction extends BasicJsonAction {
	...
	public void findMember() {
		try {
			...
			List<Map<String, Object>> memList =  DataBaseConnection.getList(sql, new Object[] { mu.getId(), mu.getId(), where, where, where, where, mu.getId() });
			if (pageInfo.getItems() == null) pageInfo.setItems(new ArrayList<Member>());
			final JSONArray jarr = new JSONArray();
			Member m = new Member();
			DecimalFormat df = new DecimalFormat("#.00");
			pageInfo.setPageSize(memList.size());//前端显示所有信息
			if(memList.size() > 0){
			for (Map<String, Object> map1 : memList) {
				...
				obj.accumulate("finishrate", map1.get("finishrate") == null ? 0 : map1.get("finishrate"))
				   .accumulate("time",map1.get("time") == null ? 0 : String.valueOf(df.format(Double.parseDouble(map1.get("time").toString()) / 60)));
				obj.accumulate("height", getInteger(set.getHeight() == null ? 0 : set.getHeight()))
				   .accumulate("weight", getDouble(set.getWeight() == null ? 0 : set.getWeight()));
				obj.accumulate("bmiHigh", set.getBmiHigh() == null ? 0 : set.getBmiHigh())
				   .accumulate("bmiLow", set.getBmiLow() == null ? 0 : set.getBmiLow());
				obj.accumulate("age", i).accumulate("heart", set.getHeart() == null ? 0 : set.getHeart());
	...
	public void findMemberHome() {
		try {
			...
			obj.accumulate("success", true).accumulate("id", id).accumulate("name", getString(m.getName())).accumulate("nick", getString(m.getNick()))
					...
					.accumulate("sex", m.getSex()).accumulate("age", i).accumulate("heart", set.getHeart()).accumulate("time", list.get(0).get("time") == null ? 0 : String.valueOf(df.format(Double.parseDouble(list.get(0).get("time").toString()) / 60)))
	...
	public void modifyPassword() {
		try {
			...
			if (flag != null && "0".equals(flag)) {
				...
				Member m = (Member) service.load(Member.class, getMobileUser().getId());
				if (m.getPassword().equalsIgnoreCase(oldPassword)) {//判断原密码是否正确
					final String password = request.getParameter("password");
					m.setPassword(password);
					service.saveOrUpdate(m);
					ret.accumulate("success", true).accumulate("message", "OK");
