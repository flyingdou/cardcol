...
function changeOrderEndTime(){
	...
	<div class="proedifour">
		...
		<p>
			1.甲方承诺从<input type="text" name="shop.orderStartTime" id="orderStartTime" class="text1" value="<s:date name="shop.orderStartTime" format="yyyy-MM-dd"/>" onchange="changeOrderEndTime();"/>起，购买乙方的  <span><s:property value="product.num"/></span>次  计次卡，成为乙方会员。</p>
		<p>
		...
		<p>
			3.该会员资格的有效期限为 <span><s:property value="product.wellNum"/></span> 月；有效期限届满后，卡内仍有剩余次数的，乙方应当提供一次展期，展期时间计算方式：剩余次数乘以 <span><s:property value="product.delayDay"/></span> 天（3-5天的合理健身频率）=展期天数。
		</p>
		<p>
			4.甲方应向乙方支付合同款共计<span><s:property value="product.cost"/></span> 元。双方同意将该合同款交付给丙方托管，并由丙方每月 <span id="shopReportDaySpan"><s:property value="shop.reportDay"/></span> 号进行上一个月甲乙双方应付费用的结算。
		</p>
		<p>
			5.甲方因自身原因提前终止合同，应向乙方支付违约金 <span><s:property value="product.breachCost"/></span> 元。乙方收到违约金后向甲方退还甲方未消费的金额。
		</p>
		<p>
