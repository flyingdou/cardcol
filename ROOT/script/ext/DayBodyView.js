Ext.calendar.DayBodyView = Ext.extend(Ext.calendar.CalendarView, {
    ...
    getEventTemplate: function() {
        if (!this.eventTpl) {
            ...
            new Ext.XTemplate(
            '<div id="{_elId}" class="{_selectorCls} {_colorCls} ext-cal-evt ext-cal-evr" style="left: {_left}%; width: {_width}%; top: {_top}px; height: {_height}px;background-color: {_bgColor};">',
            '<div class="ext-evt-bd" style="color: {_color};">', this.getEventBodyMarkup(), '</div>',
            '<div class="ext-evt-bd" style="color: {_color};">{_coach}</div>',
            ...
            '<dd class="ext-evt-bd" style="color: {_color};">',
            this.getEventBodyMarkup(),
            '</dd>',
            '<dd class="ext-evt-bd" style="color: {_color};">{_coach}</dd>',
            '<div class="ext-evt-rsz"><div class="ext-evt-rsz-h">&nbsp;</div></div>',
            '</dl>',
            '<div class="ext-cal-evb">&nbsp;</div>',
            '</div>'
...
//    	for (var o in evt) alert(o + ' = ' + evt[o]);
        ...
        data._color = getColor(data._bgColor);
        data._coach = evt[M.CoachName.name] + (evt[M.Count.name] > 0 ? ' ' + '(' + evt[M.JoinNum.name] + '/' + evt[M.Count.name] + ')' : '');
        data._colorCls = 'ext-color-3';// + evt[M.Color.name];
        data._elId = selector + (evt._weekIndex ? '-' + evt._weekIndex: '');
        data._isRecurring = evt.Recurrence && evt.Recurrence != '';
        data._isReminder = evt[M.Reminder.name] && evt[M.Reminder.name] != '';
        var title = evt[M.CourseName.name];
        data.Title = (!title || title.length == 0 ? '无标题': title);
//        data.Title = evt[M.StartDate.name].format('g:ia ') + (!title || title.length == 0 ? '无标题': title);
    ...
    renderItems: function() {
        ...
        for (; day < this.dayCount; day++) {
            ...
            for (; ev < ct; ev++) {
                ...
                Ext.apply(item, {cls: 'ext-cal-ev', _positioned: true});
                evts.push({data: this.getTemplateEventData(item), date: this.viewStart.add(Date.DAY, day)});
            }
        }
