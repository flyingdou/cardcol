Ext.calendar.CalendarView = Ext.extend(Ext.BoxComponent, {
    ...
    initComponent: function() {
    	this.initialize(this.startDate || new Date());
//        this.setStartDate(this.startDate || new Date());

        Ext.calendar.CalendarView.superclass.initComponent.call(this);

        this.addEvents({headClick: true, eventsrendered: true, eventclick: true, eventover: true, eventout: true, datechange: true, rangeselect: true, eventmove: true,
            initdrag: true,
            dayover: true,
            dayout: true
            //eventdelete: true
    ...
    setStartDate: function(start, refresh) {
        this.startDate = start.clearTime();
        this.setViewBounds(start);
        var _params = this.parameter || {};
        if (refresh == true) {
	        _params.startDate = this.viewStart.format('Y-m-d');
	        _params.endDate = this.viewEnd.format('Y-m-d');
	        this.store.load({params: _params});
        }
        if (refresh === true) {
            this.refresh();
        }
...
//            }
    ...
    movePrev: function(noRefresh) {
        ...
        return this.moveDays(0 - days, noRefresh);
    },
