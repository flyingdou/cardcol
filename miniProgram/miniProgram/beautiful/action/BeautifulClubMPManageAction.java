package miniProgram.beautiful.action;
...
import miniProgram.beautiful.constant.Constants;
import miniProgram.beautiful.util.AES;
import miniProgram.wechatAPI.PayManager;
import miniProgram.wechatAPI.PayRequest;
...
public class BeautifulClubMPManageAction extends BasicJsonAction implements SessionConstant {
	...
	public void myFooter() {
		try {
			...
			String querySign = "select s.id,m.id clubId,m.name,m.image,s.signDate,me.TOTALITY_SCORE totalityScore,"
					...
					+ " where s.f_money = 1 and s.memberAudit = " + clubId + " and s.memberSign =  " + memberId
	...
	public void editTicket() {
		try {
			...
			String querySql = "select m.id,m.name from tb_member m inner join"
					+ " tb_member_friend f on m.id = f.member where m.role = 'E' and f.friend = " + member.getId();
			List<Map<String, Object>> list = service.queryForList(querySql);
	...
	public void createClubMPOrder() {
		try {
			...
			// 保存用户信息
			member.setNick(json.getString("realName"));
			String sex = json.getInt("gender") == 0 ? "F" : "M";
			member.setSex(sex);
			order.setPayEmail(json.getString("origin"));
			...
			service.saveOrUpdate(member);
			// 支付签名
			PayRequest payRequest = new PayRequest(Constants.APPID, Constants.MCH_ID, Constants.APP_KEY,
					json.getString("openId"));
			PayManager payManager = new PayManager(request);
			String result = payManager.paySign(payRequest, order);
	...
	public void releasePriceCutdown() {
		...
		try {
			...
			// 砍价活动说明
			pcd.setRemark(param.getString("remark"));
