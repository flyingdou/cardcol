...
Ext.calendar.MonthView = Ext.extend(Ext.calendar.CalendarView, {
    ...
    getEventTemplate: function() {
        if (!this.eventTpl) {
            ...
            new Ext.XTemplate(
            '<div id="{_elId}" class="{_selectorCls} {_colorCls} {values.spanCls} ext-cal-evt ext-cal-evr" style="color: {_bgColor}">',
            body,
            '</div>'
            )
            : new Ext.XTemplate(
            '<tpl if="_renderAsAllDay">',
            '<div id="{_elId}" class="{_selectorCls} {values.spanCls} {_colorCls} ext-cal-evt ext-cal-evo" style="color: {_bgColor}">',
            '<div class="ext-cal-evm">',
            '<div class="ext-cal-evi">',
            '</tpl>',
            '<tpl if="!_renderAsAllDay">',
            '<div id="{_elId}" class="{_selectorCls} {_colorCls} ext-cal-evt ext-cal-evr" style="color: {_bgColor}">',
            '</tpl>',
            body,
            '<tpl if="_renderAsAllDay">',
            '</div>',
    ...
    getTemplateEventData: function(evt) {
        ...
        title = evt[M.CourseName.name];

        return Ext.applyIf({
            _selectorCls: selector,
            _bgColor: evt[M.Color.name],
            _colorCls: 'ext-color-3',// + (evt[M.Color.name] ? evt[M.Color.name] : 'default'),
            ...
            Title: evt[M.StartDate.name].format('g:ia ') + (!title || title.length == 0 ? '无标题': title)
