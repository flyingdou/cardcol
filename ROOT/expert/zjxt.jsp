...
<body>
	<div class="wraper" id="wraper">
		...
		<div class="box-middle">
			<a href="javascript:void(0)" @click="myPlan()" class="button button-plan">我的计划</a>
...
<script>
	...
	var vue = new Vue({
		...
		created:function(){
			this.params = location.href.split("?")[1].split("&");
			
			if(parseInt(this.params[1].split("=")[1]) > 0){
				$(".ticket").show();
			}
		...
		methods:{
			myPlan: function(){
				if(parseInt(this.params[0].split("=")[1]) <= 0){
					alert("您取消了授权，需要在“发现”的小程序页面将“王严健身专家系统”删除，重新登录授权才可以体验后续功能");
					return;
				}
				wx.miniProgram.navigateTo({url: '../plan/plan'});
			},
			goBuy: function(){
				if(parseInt(this.params[0].split("=")[1]) <= 0){
					alert("您取消了授权，需要在“发现”的小程序页面将“王严健身专家系统”删除，重新登录授权才可以体验后续功能");
