...
; (function($) {
    ...
    $.fn.bcalendar = function(option) {
        ...
        function getTitle(event) {			
            var timeshow, locationshow, attendsshow, eventshow,costs1,costs2;
            ...
            // added by lk start
            costs1 = (event[11] != undefined && event[11] != "") ? event[11] : "";
            costs2 = (event[12] != undefined && event[12] != "") ? event[12] : "";
            priceshow = (event[13] != undefined && event[13] != "") ? event[13] : "";
            sportprojectshow = (event[14] != undefined && event[14] != "") ? event[14] : "";
            // added by lk end
            var ret = [];
            if (event[4] == 1) {
            ...
            if(sportprojectshow!=""){
            	ret.push($.browser.mozilla?"":"\r\n", i18n.xgcalendar.sportproject + ":", sportprojectshow);
            }
            if (costs2 != "") {
                ret.push($.browser.mozilla?"":"\r\n", "会员:", costs2);
            }
            ...
            if(priceshow!=""){
            	ret.push($.browser.mozilla?"":"\r\n", i18n.xgcalendar.price + ":", priceshow);
            }
            // updated by lk end
            if (attendsshow != "") {
        ...
        function BuildDayEvent(theme, e, index) {
            ...
            // updated by lk start
            //p.content = e.event[1];
            var costs1 = (e.event[11] != undefined && e.event[11] != "") ? e.event[11] : "";
            var costs2 = (e.event[12] != undefined && e.event[12] != "") ? e.event[12] : "";
            var costs = "";
            if(costs2 != ""){
            	costs = "会员: " + costs2;
            }
            if(costs1 != ""){
            	costs = costs + " 非会员: " + costs1;
            }
            if(costs!=""){
            	p.content = e.event[1] + "<br>" + costs;
            }else{
            	p.content = e.event[1];
            }
            // updated by lk end
                        ...
                        lbtn.click(function(e) {
                            if (!option.EditCmdhandler) {
                                ...
                                if (option.EditCmdhandler && $.isFunction(option.EditCmdhandler)) {
                                	var data = $("#bbit-cs-buddle").data("cdata");
                                	data[2] = data[2].format();
                                	data[3] = data[3].format();
            ...
            if (buddle.length == 0) {
                ...
                lbtn.click(function(e) {
                    if (!option.EditCmdhandler) {
                        ...
                        if (option.EditCmdhandler && $.isFunction(option.EditCmdhandler)) {
                        	var _starts = $("#bbit-cal-start").val().split(" "), _ends = $("#bbit-cal-end").val().split(" ");
                        	var _start = _starts[0].toYMD() + " " + _starts[1];
                        	var _end = _ends[0].toYMD() + " " + _ends[1];
                            option.EditCmdhandler.call(this, ['0', $("#bbit-cal-what").val(), _start, _end, $("#bbit-cal-allday").val()]);
                        }
                        $("#bbit-cal-buddle").css("visibility", "hidden");
                        realsedragevent();
                    }
        ...
        function dragEnd(e) {
            if (_dragdata) {
                var d = _dragdata;
                var _key = d.data ? d.data[0] : 0, _start, _end;
                switch (d.type) {
                    case 1: //day view
...
//                if (_key && _key > 0) {
//                    if (option.DragCmdhandler && $.isFunction(option.DragCmdhandler)) {
//                        option.DragCmdhandler.call(this, [_key, _start, _end]);
//                    }
//                }
                d = _dragdata = null;
                $('body').noSelect(false);
