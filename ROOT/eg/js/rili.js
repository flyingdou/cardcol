var currDT; 
var aryDay = new Array("周日","周一","周二","周三","周四","周五","周六");//显示星期  
...
function initDate() {
	...
	var objTB1 = document.getElementById("mytable1");//取得表格对象
	for(var i=0;i<7;i++) {
		tdDT = getDays()[i];
		if(tdDT.toLocaleDateString() == currDT.toLocaleDateString()) {  
		objTB.rows[0].cells[i].style.color = "#1296db";//currDT突出显示 
			objTB1.rows[0].cells[i].style.color = "#1296db";//currDT突出显示 
			//将背景设置为灰色
			objTB.rows[0].cells[i].style.backgroundColor = "white";
		}  
		dw = tdDT.getDay();//星期几     
		objTB.rows[0].cells[i].innerHTML = tdDT.getMonth()+1 + "-" + tdDT.getDate();//显示 
		objTB1.rows[0].cells[i].innerHTML =aryDay[dw];//显示    "星期"的显示
	} 
	//重新赋值
...
function nextWeek() {
	...
	for(var i=0;i<7;i++) {
		...
		/* if(tdDT.toLocaleDateString()==currDT.toLocaleDateString()) {  
			...
			objTB1.rows[0].cells[i].style.color = "red";//currDT突出显示
		}   */
		dw = tdDT.getDay();//星期几
		objTB.rows[0].cells[i].innerHTML = tdDT.getMonth()+1 + "-" + tdDT.getDate() ; //显示 
	} 
