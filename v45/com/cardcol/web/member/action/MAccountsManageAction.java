...
public class MAccountsManageAction extends BasicJsonAction {
	...
	public void ticketAll() {
		try {
			...
			if ("0".equals(orderType)) {
				...
				List<MemberTicket> tickets = (List<MemberTicket>) service
						.findObjectBySql(
				...
				for (final MemberTicket mt : tickets) {
					...
					if (ymd.format(c.getTime()).compareTo(
						...
						if (mt.getTicket().getScope() != null) {
							...
							String[] ranges = mt.getTicket().getScope()
									.split(",");
							for (int i = 0; i < ranges.length; i++) {
								sb.append(sb.length() <= 0 ? "" : ",").append(
										WebConstant.getType(ranges[i].trim()));
							}
				...
				if("8".equals(orderType))  orderType ="7";
				List<Map<String, Object>> mts = service.queryForList("SELECT a.id tid,a.active_date as activeDate,a.status,b.* FROM tb_member_ticket a LEFT JOIN tb_ticket b ON a.ticket = b.id WHERE DATE_ADD(a.active_date, INTERVAL b.period DAY) >= CURRENT_DATE() and a.status = ? and a.member = ? and b.scope like ?  order by a.active_date  desc ",STATUS_TICKET_USE,  getMobileUser().getId(), "%" + orderType + "%");
				...
				for (final Map<String, Object> mt : mts) {
					...
					final JSONObject obj = new JSONObject();
					obj.accumulate("id", mt.get("tid"))
							.accumulate("name", mt.get("name"))
							.accumulate("price", mt.get("price"))
							.accumulate( "status", (int)mt.get("status")== STATUS_TICKET_USE ? "可用" :(int)mt.get("status") == STATUS_TICKET_USED ? "已使用": "已失效")
							.accumulate("becomeDate",ymd.format(calendar.getTime()) );
	...
	public void activeTicket() {
		try {
			...
			final List<?> list = service.findMemberTicketByCode(activeCode,
					getMobileUser().getId());
			if (list.size() > 0)
